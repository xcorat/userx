<!-- MemeSubmissionForm component for reusable meme submission form -->
<script lang="ts">
	...
	import { Upload, Eye, AlertTriangle } from 'lucide-svelte';
	import type { CreateMemeDTO } from '$lib/models/meme.model';

	interface Props {
		onSubmit: (data: Omit<CreateMemeDTO, 'submittedBy'>) => Promise<void>;
		onCancel: () => void;
		disabled?: boolean;
	...
	async function handleSubmit(e: Event) {
		...
		if (!validateForm() || !tokenAvailable) return;
...
<!-- Token Status Banner -->
<div class="token-status" class:warning={!tokenAvailable}>
	...
	<div class="token-text">
		{#if tokenAvailable}
			...
			<span>You can submit {tokensRemaining} meme today</span>
		{:else}
...
<Card.Root class="submission-form">
	...
	<form onsubmit={handleSubmit}>
		<Card.Content class="form-content">
			...
			<div class="field-group">
				...
				<Input
					...
					bind:value={imageUrl}
					disabled={isSubmitting || disabled || !tokenAvailable}
		...
		<Card.Footer class="form-footer">
			...
			<Button 
				type="submit" 
				disabled={isSubmitting || disabled || !tokenAvailable || !imageUrl.trim()}
				class="submit-button"
			>
				{#if isSubmitting}
					Submitting...
