...
class DIContainer {
	...
	private static get memeRepo(): IMemeBallRepository {
		if (!this._memeRepo) {
	...
	static getMemeService(): MemeService {
		if (!this._memeService) {
			this._memeService = new MemeService(this.memeRepo, this.userRepo);
		}
